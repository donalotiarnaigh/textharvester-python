    ...
    for record in records:
        if isinstance(record, dict):
            if 'error' not in record:
                ...
                print(f"Valid record added: {record}")
            else:
...
def process_json_files(folder_path):
                ...
                try:
                    ...
                    try:
                        ...
                        if isinstance(records, list):
                            ...
                            print(
                                f"Records from {json_file} added to the collection")
                        elif isinstance(records, dict):
                            all_records.extend(collect_records([records]))
                            print(
                                f"Single record from {json_file} added to the collection")
                        else:
                            print(
    ...
    sorted_records = sorted(
        all_records, key=lambda x: int(x['memorial_number']))
